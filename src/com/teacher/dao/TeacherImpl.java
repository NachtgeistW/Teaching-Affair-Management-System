...
public class TeacherImpl implements TeacherDao {
	...
	public TeacherQuery getCurse(String cid, String sid) {
		...
		sql = "select * from course_info,teacher_info,teacher_course where teacher_course.tteacherid = ? and teacher_course.cid=? AND course_info.cid = teacher_course.cid and teacher_course.tteacherid = teacher_info.tteacherid";
		try {
            con = ConnectionUtil.getConnection();
            pt = con.prepareStatement(sql);
            pt.setString(1, sid);
	...
	public int teacherCurseUpdate(TeacherQuery curse) {
		...
		ResultSet b = null;
		Boolean c = true;
		sql = "SELECT * FROM student_course,teacher_course where teacher_course.cid=? and student_course.cid=teacher_course.cid and student_course.sterm=teacher_course.cterm";
        try {
            con = ConnectionUtil.getConnection();
            pt = con.prepareStatement(sql);
            pt.setString(1, curse.getCid());
            b = pt.executeQuery();
            c = b.next();
        } catch (SQLException e) {
            // TODO Auto-generated catch block
        ...
        if(c)
        {
        	...
        	sql = "UPDATE teacher_course SET teacher_course.cterm=? where teacher_course.cid=?";
	        try {
	            con = ConnectionUtil.getConnection();
	            pt = con.prepareStatement(sql);
	            pt.setString(1, curse.getCterm());
